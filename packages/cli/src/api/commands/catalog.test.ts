...
describe('SCHEMA_MAX_LIMIT / SCHEMA_DEFAULT_LIMIT', () => {
  it("matches the server's own SCHEMA_MAX_LIMIT, so the two cannot silently drift", () => {
    ...
    // which refuses non-canonical input the way route ids always have. This
    // regex tracks the new spelling. That it had to be updated at all is the
    // cost of reading a default out of source text -- accepted deliberately,
    // because the alternative is the CLI trusting a hand-copied number.
    const defaultMatch =
      /limit:\s*countParam\(\{\s*min:\s*1,\s*max:\s*SCHEMA_MAX_LIMIT,\s*fallback:\s*(\d+)\s*\}\)/.exec(
        routesSrc,
      )
