...
describe('shared member projection', () => {
  ...
  // Pins the segment members query's SQL IN FULL, byte-for-byte. Every
  // segment member walk in the product goes through this code, so any
  // change to it -- a column added or removed, a JOIN reordered, even
  // whitespace -- must be a DELIBERATE, VISIBLE line in a diff rather than
  // an incidental side effect of touching something nearby elsewhere in
  // the compiler. A substring check (`toContain`) cannot do this job: it
  // would not notice a fragment moved into a different clause, a JOIN
  // silently dropped, or a column renamed to something that still
  // contains the old text. This is NOT a "this SQL may never change"
  // guard -- it started as one extraction's proof that a refactor changed
  // nothing, and that proof's job is spent; every deliberate change to
  // the member projection re-captures this string against the current
  // compiler with the same inputs, most recently for `identified`
  // (base.ts). A failure here means: read the diff, and if the change is
  // the one intended, re-capture -- not "restore the old string".
