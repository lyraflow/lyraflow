 ...
 * failure -- because an unrevoked blob URL holds the whole buffered export
 * alive in memory for the rest of the tab's life, which is the same
 * liability the size ceiling exists to bound, reached by a different route.
 *
...
export function ExportButton(props: {
  ...
  const { client, projectId, personId, eventCount } = props
    ...
    return (
      <div className="flex flex-col gap-2 text-sm">
        <p className="text-muted-foreground">
          This person has more than {EXPORT_MAX_EVENTS} events -- past what this screen can safely
          buffer into one download. Their data is still fully readable through the API, which
          streams the export instead of buffering it:
        </p>
  ...
  function handleExport() {
    ...
    client
      ...
      .finally(() => {
        ...
        // resolved with a blob but failed to click/append (or one that
