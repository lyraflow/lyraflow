 ...
 *
 * `[&_option]:bg-popover [&_option]:text-foreground` is here because the open-list
 ...
 * puts the option's parent one level below the select, not directly below it.
 * `[&>option]` compiles to `select > option` and never matches those options at
 * all -- the exact gap the two call sites this component exists for would have
 * hit. `[&_option]` compiles to `select option`, reaching an option at either
 * depth.
 */
export function NativeSelect({
  ...
  return (
    <div className={cn('relative inline-flex min-w-0 items-center', containerClassName)}>
      <select
        className={cn(
          'h-9 min-w-0 max-w-full appearance-none rounded-md border border-input bg-transparent py-2 pr-8 pl-3 text-foreground text-sm shadow-xs outline-none transition-[color,box-shadow] focus-visible:border-ring focus-visible:ring-[3px] focus-visible:ring-ring/50 disabled:cursor-not-allowed disabled:opacity-50 aria-invalid:border-destructive aria-invalid:ring-destructive/20 dark:bg-input/30 dark:hover:bg-input/50 dark:aria-invalid:ring-destructive/40 [&_option]:bg-popover [&_option]:text-foreground',
          className,
        )}
        {...props}
      >
