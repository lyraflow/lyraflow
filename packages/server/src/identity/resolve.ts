...
export function resolvedPersonExpr(opts: { database: string; alias?: string }): string {
  ...
  // convertible to Int64" (confirmed against a live ClickHouse; the four
  // shape-only unit tests below do not catch this, only the live-service
  // suite in this file does).
  const stage1 =
    `if(${userId} != '', ${userId}, ` +
