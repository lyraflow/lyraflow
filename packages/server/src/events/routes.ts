...
import { type Breakdown, BreakdownError, parseBreakdown } from './breakdown.js'
import { FeedCursorError, decodeFeedCursor, encodeFeedCursor } from './cursor.js'
import {
  STATS_DEFAULT_WINDOW_MS,
  STATS_INTERVALS,
  STATS_MAX_BUCKETS,
  StatsQueryError,
  runStats,
...
export { STATS_DEFAULT_WINDOW_MS, STATS_INTERVALS, STATS_MAX_BUCKETS }
...
export function registerEventsRoutes(app: FastifyInstance, deps: EventsDeps): void {
  ...
  app.get('/v1/events/stats', async (req, reply) => {
    ...
    try {
      const body = await runStats({ ch, database }, project, {
